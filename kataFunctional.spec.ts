...
const addMatches = ({ players, tours }: Session, fieldCount: number): Tour => {
  return BySimpleWhomPlayedLeast(players, tours, fieldCount);
};

const setMatch1Score = (tours: Tour[]): MatchResult =>
  [MatchScore.Lose, MatchScore.Win].map(
    (matchScore: MatchScore, index): PlayerResult => ({
      nom: tours[0][0][index].nom,
      score: matchScore
    })
  ) as MatchResult;

const setMatch2Score = (tours: Tour[]): MatchResult =>
  [MatchScore.Lose, MatchScore.StrongWin].map(
    (matchScore: MatchScore, index): PlayerResult => ({
      nom: tours[0][1][index].nom,
      score: matchScore
    })
  ) as MatchResult;

const setScore = ({ players, tours }: Session): Session => {
  return {
    players,
    tours: [[[...setMatch1Score(tours)], [...setMatch2Score(tours)]]]
  };
};

describe('add score after every tour', () => {
  it('should set score at the end to first Tour', () => {
    ...
    expect(setScore(sessionNotPlayed)).toEqual(expectedSessionPlayed);
  });
});
